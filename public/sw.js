...
self.addEventListener("notificationclick", (event) => {
  ...
  if (event.action === "view") {
    event.waitUntil(
      clients.openWindow(
        "/payment/" + event.notification.data?.paymentId || "/",
      ),
    );
  }
});
...
async function retryFailedTransactions() {
  try {
    ...
    const failedTransactions = JSON.parse(
      localStorage.getItem("failed_transactions") || "[]",
    );
      ...
      try {
        ...
        if (response.ok) {
          ...
          localStorage.setItem(
            "failed_transactions",
            JSON.stringify(updatedFailedTransactions),
          );

          console.log(
            "Background sync: Transaction retry successful",
            transaction.id,
          );
        }
      } catch (error) {
        console.error(
          "Background sync: Transaction retry failed",
          transaction.id,
          error,
        );
      }
